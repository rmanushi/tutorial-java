public class NumberMethods
{
	public static void main(String[] args) 
	{
		
	}
	public static int minimumInt (int num1, int num2)
	{
		
	}
	public static int median (int num1, int num2, int num3)
	{
		
	}
	public static double average (int num1, int num2, int num3, int num4, int num5)
	{

	}
	public static boolean isEven (int num)
	{

	}
	public static int sumAllNumbersUpTo (int numberUpTo)
	{

	}
	public static int sumEvenNumbersUpTo (int numberUpTo)
	{

	}
	public static double percentage (int num, int numAsPercentage)
	{

	}
	public static double solveEquation (int a, int b)
	{
		
	}
}